...
output_path = os.path.join(os.path.dirname(__file__), "data", "iperf3_results.json")
...
result = subprocess.run(
    ["ping", "-c", PING_COUNT, local_ip], capture_output=True, text=True
)
...
with open(path, "r", encoding="utf-8") as file:
    ...
    for server in data:
        ...
        try:
            result = subprocess.run(
                ["ping", "-c", PING_COUNT, server["IP/HOST"]],
                capture_output=True,
                text=True,
                check=True,
            )
            ...
            print(
                f"Results: {result_data['max_rtt']}, {result_data['min_rtt']}, {result_data['mean_rtt']}"
            )
